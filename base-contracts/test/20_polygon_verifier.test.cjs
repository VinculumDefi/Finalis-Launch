...
async function build(opts = {}) {
  ...
  const borLogs = [lockLog];
  if (opts.includeDetail !== false) borLogs.push(detailLog(VAULT, VAULT_LOCK_ID));
  const borReceipt = hex(buildReceipt({ status: opts.borStatus ?? 1, logs: borLogs }));
