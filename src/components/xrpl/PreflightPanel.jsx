...
import { computeFee } from '@/lib/vfXrplLockEngine';
import { XRPL_DROPS_PER_XRP } from '@/lib/vfXrplAuthority';

function fmtDrops(drops) {
  return `${drops.toString()} drops (${(Number(drops) / XRPL_DROPS_PER_XRP).toFixed(6)} XRP)`;
}
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-xs text-[#6b6b65]">
        ...
        {handshake && ' This is a Trust-Building Handshake (1 hour). XRPL uses 1-use allowance (Q.2).'}
      </p>

      {feePreview && feePreview.ok && (
        <div className="rounded-md border border-[#18324b]/20 bg-[#18324b]/[0.03] p-3 text-xs font-mono">
          <span className="text-[#6b6b65]">Fee (VF-COM-011):</span> {fmtDrops(feePreview.fee)} ({(feePreview.bps / 100).toFixed(2)}%)
          <br />
          <span className="text-[#6b6b65]">Principal (VF-COM-012):</span> {fmtDrops(feePreview.principal)}
        </div>
      )}
