// =============================================================================
// Solana Commitment Vault Lock — Domain Engine
//
...
//
// This module implements the protocol domain logic for the Solana environment:
...
// No value is fabricated. No Cosmos-derived logic is reused (clean-room rule).
// =============================================================================
...
import {
  ...
  LOCK_STATES,
  ATTEMPT_STATES,
  SOLANA_HANDSHAKE_ALLOWANCE,
} from './vfRevision6Authority';
import { SOLANA_REGISTRY } from './vfSolanaRegistry';
...
// VF-REG-001: "The approved asset registry is the sole source of recognized external
// asset identities for Commitment Vault Locks."
export function findSolanaAsset(symbol) {
  return SOLANA_REGISTRY.find((a) => a.symbol === symbol) || null;
}
...
// Solana is account-model → release destination is a Solana address (base58, ~32-44 chars).
// Phase 1: format check only. Full validation requires the deployed chain (VF-DEP-001).
...
// with the source account." Solana is account-model.
// identity = (source_environment_id, source_account)
export function buildHandshakeIdentity(sourceAccount) {
  if (!sourceAccount) return null;
  return `(${SOLANA_ENVIRONMENT.name}, ${sourceAccount})`;
}
...
// Formula: output = Verified Gross USD Value × emission rate × asset multiplier × duration multiplier
export function computeOutput({
  verifiedGrossUsdMicro, // 18-decimal fixed-point USD value
  outputToken,
  assetClass,
  durationSecs,
  daysSinceLaunch,
  ...
  return {
    ok: true,
    output: step, // smallest protocol-token units (18 decimals)
    emissionRate,
...
// fee_usd = floor(verified_gross_usd × fee_units / gross_units)
// credit = floor(fee_usd × 60 / 100)
...
export function validateLockRequest({
  ...
  sourceAccount,
  grossAssetUnits,
  verifiedGrossUsdMicro, // 18-decimal fixed-point (SIMULATION: price lookup is simulated)
  daysSinceLaunch,
  ...
  // VF-REG-001: asset must be in the approved Solana registry
  const asset = findSolanaAsset(assetSymbol);
  if (!asset) {
    errors.push(`VF-REG-001: asset "${assetSymbol}" is not in the approved Solana registry`);
    checks.push({ id: 'VF-REG-001', pass: false });
  } else {
    checks.push({ id: 'VF-REG-001', pass: true, detail: `${asset.symbol} · row ${asset.registry_row}` });
  }
  ...
  // VF-PRI-003: valid release destination (Solana address)
  if (!validateSolanaAddress(releaseDestination)) {
    errors.push('VF-PRI-003: invalid release destination (Solana address format)');
  ...
  if (!sourceAccount || !validateSolanaAddress(sourceAccount)) {
    errors.push('VF-COM-005: invalid source account (Solana address format)');
  ...
  if (!feeResult.ok) {
    ...
    checks.push({
      id: 'VF-COM-011/012',
      pass: true,
      detail: `fee=${feeResult.fee.toString()} principal=${feeResult.principal.toString()}`,
    });
  }
  ...
  if (asset && feeResult.ok && ['VCLM', 'CHONX'].includes(outputToken)) {
    ...
    if (!output.ok) {
      ...
      const capCheck = checkHardCap(
        outputToken,
        output.output,
        outputToken === 'VCLM' ? cumulativeVclmIssued || 0n : 0n, // CHONX issued tracked separately
      );
      if (!capCheck.ok) {
